...
<section class="uk-height-1-1 uk-padding-small sprod">
    ...
    <div class="uk-height-1-1 uk-flex uk-flex-wrap uk-flex-around uk-padding-remove-bottom uk-padding-remove-horizontal">
        ...
        <div class="uk-card uk-card-default uk-card-body uk-width-1-5@m uk-flex
            ...
            <a href="{{ route('seller.product.view', $prod->slug) }}" class="uk-flex uk-flex-center">
                <img src="{{ isset($prod->coverPhotos) ? asset('uploads/products/' . $prod->coverPhotos[0]->name) : asset('images/icons/no_preview.png') }}" uk-img />
            </a>
            ...
            <div class="uk-flex uk-flex-between uk-card-footer uk-padding-remove-bottom uk-padding-remove-horizontal">
                <a href="{{ route('seller.product.edit', $prod->id) }}" type=" button" class="uk-text-warning">Edit</a>

                <form action="{{ route('product.destroy', $prod->id) }}" id="{{ 'removeProduct' . $prod->id }}" method="post">
                    @csrf
                    <a href="#" onclick="document.getElementById('{{ 'removeProduct' . $prod->id }}').submit()" class="uk-margin-left uk-link-heading sdetail-remove uk-text-danger uk-text-bold">Remove</a>
                </form>
            </div>
        </div>
        @endforeach
